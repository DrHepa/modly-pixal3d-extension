...
inference on other platforms remain **UNTESTED**.
...
Normal setup does not download weights. Use Modly Models to download the shared Hugging Face groups. On MV or WorldSculpt load/first generation, the extension first fully validates the node's non-NAF shared assets, then downloads only the NAF checkpoint from `https://github.com/valeoai/NAF/releases/download/model/naf_release.pth` if NAF alone is absent. Missing base, MV, or WorldSculpt adapter files fail with an actionable Models UI repair error before bootstrap. The manual safety command is `python3 setup.py --bootstrap-auxiliary-assets --workspace-root <extension-dir> --json`; it downloads only NAF. If an existing NAF file fails size or SHA-256 validation, automatic replacement is deliberately disabled: run `python3 setup.py --bootstrap-auxiliary-assets --force-auxiliary-assets --workspace-root <extension-dir> --json`. `local`, `offline`, and `strict` single-view auxiliary modes still require NAF to be present and never start that download.
