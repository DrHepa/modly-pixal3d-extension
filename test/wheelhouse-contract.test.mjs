...
test('windows cp311 cuda124 NATTEN candidate workflow is manual, exact-stack, and artifact-only', () => {
  ...
  assert.match(workflow, /sub-packages: '\["nvcc", "cudart-dev"\]'/)
  ...
  assert.match(workflow, /NATTEN_VERSION: v0\.17\.5/)
  assert.doesNotMatch(workflow, /NATTEN_VERSION: v0\.21\./)
  ...
  assert.match(workflow, /import json, natten, torch/)
  assert.match(workflow, /HAS_LIBNATTEN/)
  assert.match(workflow, /NATTEN-CANDIDATE\.json/)
  assert.match(workflow, /verification\.json/)
  assert.match(workflow, /actions\/upload-artifact@v4/)
  assert.match(workflow, /natten-windows-x64-cp311-cuda124-candidate/)
  ...
  assert.match(recipe, /SHI-Labs\/NATTEN` tag `v0\.17\.5`/)
