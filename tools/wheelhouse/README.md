...
- It patches vendored CUTLASS C++17 detection to consider `_MSVC_LANG`, because MSVC may leave `__cplusplus` below `201703L` while compiling as C++17, hiding `cutlass::platform::is_unsigned_v` from CUDA translation units.
- It patches NATTEN `helpers.h` to replace the C++ alternative token `not` with `!` in the `CHECK_CONTIGUOUS(x)` sparse-tensor check, because the CUDA/MSVC compile path rejects the alternative token after macro expansion in generated `.cu` objects.
