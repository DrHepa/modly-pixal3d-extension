...
- It patches NATTEN `helpers.h` to replace the C++ alternative token `not` with `!` in `logsumexp.is_sparse()` checks, because the CUDA/MSVC compile path rejects the alternative token in generated `.cu` objects.
